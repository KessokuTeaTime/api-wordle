...
pub mod layers {
    ...
    static_lazy_lock! {
        ...
        .allow_origin(AllowOrigin::async_predicate(|origin: HeaderValue, _request_parts: &request::Parts| {
            async move {
                let config = CorsConfig::read().unwrap_or_default();
                tracing::trace!("origin: {origin:?}, allowed CORS origins: {:?}", config.origins);
